...
ndk_set_var_hash_code(ngx_http_script_engine_t *e)
{
    ...
    p = (u_char*)ngx_palloc(e->request->pool, svs->size);
    if (p == NULL) {
        e->ip = ndk_http_script_exit;
        e->status = NGX_HTTP_INTERNAL_SERVER_ERROR;
        return;
...
ndk_set_var_name(ndk_set_var_info_t *info, ngx_str_t *varname)
{
    ...
    cf = info->cf;
    rlcf = (ndk_http_rewrite_loc_conf_t*)ngx_http_conf_get_module_loc_conf(cf, ngx_http_rewrite_module);
    ...
    ndk_set_var_t *filter)
{
    ...
    if (filter == NULL) {
        return (char*)"no filter set";
    }

    switch (filter->type) {
    case NDK_SET_VAR_BASIC:

        sv = (ndk_set_var_code_t*)ngx_http_script_start_code(cf->pool, &rlcf->codes,
    ...
    case NDK_SET_VAR_DATA:
        ...
        svd = (ndk_set_var_data_code_t*)ngx_http_script_start_code(cf->pool, &rlcf->codes,
    ...
    case NDK_SET_VAR_MULTI_VALUE:
        ...
        svs = (ndk_set_var_size_code_t*)ngx_http_script_start_code(cf->pool, &rlcf->codes,
    ...
    case NDK_SET_VAR_MULTI_VALUE_DATA:
        ...
        svsd = (ndk_set_var_size_data_code_t*)ngx_http_script_start_code(cf->pool, &rlcf->codes,
...
ndk_set_var_filter_value(ndk_set_var_info_t *info, ndk_set_var_t *filter)
{
    ...
    if (v->set_handler) {
        vhcode = (ngx_http_script_var_handler_code_t*)ngx_http_script_start_code(cf->pool, &rlcf->codes,
    ...
    vcode = (ngx_http_script_var_code_t*)ngx_http_script_start_code(cf->pool, &rlcf->codes,
...
ndk_set_var_core(ngx_conf_t *cf, ngx_str_t *name, ndk_set_var_t *filter)
{
    ...
    p = (char*)ndk_set_var_name(&info, name);
    if (p != NGX_CONF_OK) {
        return p;
    }
